...
def main():
    ...
    point_coords = np.array([[700, 380]]) # Ensure this points to a pallet
    ...
    # The pallet is on the floor. Therefore, the FRONT edge of the pallet 
    # corresponds exactly to the BOTTOM-most pixels of the mask.
    ...
    # Get the line equation: X = m * Y + c
    m = ransac.estimator_.coef_[0]
    c = ransac.estimator_.intercept_
    
    # Isolate Inliers (the actual straight front edge)
    inlier_mask = ransac.inlier_mask_
    inlier_Y = Y[inlier_mask]
    
    # CALCULATE FRONT CENTER (Midpoint of the visible line segment)
    min_Y, max_Y = np.min(inlier_Y), np.max(inlier_Y)
    est_front_y = (min_Y + max_Y) / 2.0
    est_front_x = m * est_front_y + c  # Plug Y back into line equation to get X
    
    # Calculate Yaw (Normal vector pointing toward forklift)
    nx, ny = -1, m
    ...
    # Find GT Front Center (The midpoint of the edge closest to forklift)
    ...
    # Sort corners by X (closest to forklift)
    ...
    # Draw GT Box (Green) for reference
    draw_3d_obb(vis_img, gt_cx, gt_cy, gt_yaw, hx, hy, 0.15, T_world2cam, intrinsics, color=(0, 255, 0))
    
    # Draw TARGET POINTS (Green = GT Front Center, Red = Estimated Front Center)
    ...
    print(f"Success! Check '{os.path.abspath(RESULTS_DIR)}/04_front_center_benchmark.png'")
