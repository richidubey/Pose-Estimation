...
PALLET_POINTS =[
    (184, 391),  # Pallet 1
    (446, 396),  # Pallet 2 (With Box)
    (643, 380),  # Pallet 3
    (899, 391),  # Pallet 4
    (1176, 402)  # Pallet 5 (With Box)
]
...
def main():
    ...
    # Pre-load Ground Truth Obbs
    obbs_df = pd.read_csv(OBB_PATH)

    # --- NEW FIX: Filter Dataframe to ONLY include Pallets! ---
    # This ignores SM_CubeBox and other rigid bodies that might sit on top of the pallets
    pallets_only_df = obbs_df[obbs_df['prim_path'].str.contains('Pallet', case=False, na=False)].copy()

    for pallet_idx, (px, py) in enumerate(PALLET_POINTS, start=1):
        print(f"\n==========================================")
        print(f"Processing Pallet {pallet_idx}/{len(PALLET_POINTS)} at Coords: ({px}, {py})")
        
        img_name_str = f"{IMG_IDX:05d}"
        pallet_dir = os.path.join(RESULTS_DIR, img_name_str, str(pallet_idx))
        ...
        plt.title(f"Pallet {pallet_idx} - 3D Projected Front Edge")
        plt.axis('equal')
        plt.grid(True)
        plt.legend()
        ...
        plt.xlabel('X (World Coordinates)')
        plt.ylabel('Y (World Coordinates)')
        plt.title(f"Pallet {pallet_idx} - RANSAC Edge Fitting")
        plt.axis('equal')
        plt.grid(True)
        plt.legend()
        ...
        # Match against PALLETS ONLY, completely ignoring the boxes
        distances = np.sqrt((pallets_only_df['cx'] - est_front_x)**2 + (pallets_only_df['cy'] - est_front_y)**2)
        gt_row = pallets_only_df.iloc[distances.argmin()]
    ...
    print("\nAll pallets processed successfully!")
